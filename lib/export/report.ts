...
export async function generateExportBundle(request: ExportRequest): Promise<ExportBundle> {
  ...
  if (scope === "proposals" || scope === "proposal") {
    const dealById = new Map(opportunities.map((deal) => [deal.id, deal] as const));
    const currency = organization?.default_currency ?? "PKR";

    if (request.format === "pdf") {
      const doc = new PDFDocument({ size: "A4", margin: 44 });
      doc.fontSize(18).fillColor("#0f172a").text("FINLONEXA CRM", { align: "left" });
      doc.fontSize(14).text(scope === "proposal" ? "Proposal Export" : "Proposals Export");
      doc.fontSize(9).fillColor("#475569").text(`Generated: ${new Date().toLocaleString()}`);
      doc.text(`Generated By: ${userData.user.email ?? "User"}`);
      doc.text(`Report Period: ${periodLabel}`);
      doc.moveDown(1);
    ...
    proposals.forEach((p) => {
      const deal = p.deal_id ? dealById.get(p.deal_id) : undefined;
      sheet.addRow({
        proposal_number: safeText(p.quote_number ?? p.id ?? "—"),
        opportunity: safeText(deal?.name ?? "—"),
        ...
        owner: safeText(p.created_by ? (owners[p.created_by]?.name ?? "Owner") : "—"),
      });
    });
    sheet.getRow(1).font = { bold: true };
    sheet.views = [{ state: "frozen", ySplit: 1, xSplit: 0 }];
    const buffer = await workbook.xlsx.writeBuffer();
    return {
      filename: `${sanitizeFileName(scope === "proposal" ? "FinloNexa-Proposal" : "FinloNexa-Proposals")}-${new Date().toISOString().slice(0, 10)}.xlsx`,
      buffer: Buffer.from(buffer),
      contentType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    };
  }
