...
export async function listSalesRegions(
  ...
  const { data, error } = await db
    .from("sales_regions")
    .select(
      "id, organization_id, name, code, description, status, created_at, organization_members(sales_region_id, reports_to_user_id, roles(name), profiles!reports_to_user_id(full_name))",
    )
    .eq("organization_id", organizationId)
